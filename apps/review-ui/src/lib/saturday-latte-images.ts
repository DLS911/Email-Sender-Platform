...
- **ROADS CONNECT SOMEWHERE.** A road / path / driveway / trail in the frame either continues past the frame edge, curves toward a visible destination (a house, a treeline, a horizon), or clearly leads down to water/beach. It does NOT just end in the middle of an open field with nothing on the other side, and it does NOT terminate in a wall of fog for no reason.
- **BUILDINGS HAVE COMPLETE FACADES AND FOUNDATIONS.** No walls that stop halfway up. No houses floating above the ground with no foundation visible. No storefronts with a roof but only two walls. If a building's other sides would be off-frame, that's fine — but the visible sides must be complete and touch the ground realistically.
- **PIERS, DOCKS, JETTIES.** These are ATTACHED to the shore at one end and extend over water. A pier floating in the middle of open water with no shore connection is nonsense. A dock must have visible pilings supporting it and a visible connection back to a shoreline, boathouse, or boardwalk.
- **RAIL, POWER, FENCE LINES.** These are continuous. Rails don't just stop mid-yard. Fences don't have a single 6-foot section standing alone in a field. Power lines follow a pole line, not one pole in isolation.
- If the writer's prompt describes a landscape feature that would require impossible or half-built geometry to render, tighten the frame instead — a close-up of one structural detail, or a ground-level angle that avoids showing the far side entirely. A believable partial view beats an ambitious wide shot with a dead-end structure.

**BACKGROUND / SETTING SIMPLICITY — HARD RULE.** The subject in the frame (bottle, book, film poster, product, dish, car) is the WHOLE point. Backgrounds must be simple and clean so Gemini doesn't hallucinate broken details. Specifically banned in the background of every reference-preserve frame:
- **Electrical outlets, wall sockets, light switches, plug prongs, extension cords.** Gemini renders these with wrong pin counts, mangled slot geometry, floating outlets on brick walls, or duplex outlets that morph into single circles. If the setting is a wall behind a bottle / product / book, that wall is BLANK — plaster, wood paneling, brick, or a plain painted surface. No outlets, no switches, no thermostats, no wall-mounted anything.
- **Additional books, additional posters, additional printed matter behind the primary subject.** A book on a table does not need "another book behind it" — Gemini renders the spine text garbled every time. A film poster does not need a "framed book on the wall behind it." A bottle does not need "a magazine open on the counter behind." One printed subject per frame; everything else is objects with no legible text.
- **Signage, chalkboards, menu boards, price cards, receipts, letterforms, numbers on packaging near the primary subject.** These render as garbled almost-text and read as AI-fake immediately.
- **TVs, monitors, laptops, tablet screens, phone screens behind the primary subject.** Screens render with impossible pixel content. If a phone is required (podcast/album context), it can appear as the primary subject with clean album art — but never as a background prop.
- **Complex geometry: gallery walls with 8 framed pieces, cluttered shelving with 30 objects, restaurant walls covered in memorabilia.** Gemini renders complex arrangements with impossible perspective, floating frames, and repeated garbled details. Keep the background one plain surface + at most 1-2 simple objects (a lamp, a plant, a plain jar).

Default background for every reference-preserve edit: a single plain surface (blank wall, wood counter, marble slab, linen tablecloth) with warm directional light and NO wall-mounted anything. When in doubt, LESS in the background — a boring wall beats a busy AI-glitched wall.

**FOOD IMAGES: NO INSTRUMENTS IN THE FOOD (default).** Do NOT render a probe thermometer / instant-read thermometer / meat probe / cake tester / any measuring instrument stuck INTO the food, IN the pan next to the food, or ON the plate. Cooks measure off-camera. The ONLY exception is when the writer's Host's Corner move title explicitly names the temperature or the probing itself (e.g. "The 130°F Slow-Cook Salmon" or "The Thermometer Method for Custard") — in every other case the frame is food AT REST with no instruments in it. A ribeye on a plate does not need a probe. A slice of salmon on rice does not need a probe. Bread does not need a probe.

**FOOD IMAGES: NO WIRED EARBUDS OR HEADPHONES.** For "Worth Listening" (podcast, album) tasting menu picks: DO NOT render wired earbuds, wired headphones, coiled cables, or an AirPod stem draped across a phone. Gemini almost always renders earbuds with impossible cable geometry (coils that twist through themselves, buds attached to no cable, cables that fade into background). Instead show a plausible LISTENING CONTEXT without the ear-device visible in frame: a kitchen with a Bluetooth speaker on a shelf, a car dashboard at dawn with a phone in the cradle running the podcast/album, a workout bench with a phone playing, an open journal + a coffee cup + a phone showing the podcast art, a running trail with a smartwatch. The device MAY appear off-frame or as a phone screen showing the album/podcast art clearly — but the earbuds themselves are banned.
...
async function callGemini(
  ...
  // Every Gemini image call retries up to 3 attempts on transient
  // failure (5xx, blocked-response, empty-parts, network error).
  // Austin: "have the 3 retryies for all of them."
    ...
    try {
      ...
      for (const part of outParts) {
        if (part.inlineData?.data) {
          ...
          if (attempt > 1) {
            console.info("latte.gemini_call_succeeded_after_retry", { attempt });
          }
      ...
      lastErr = err;
      console.warn("latte.gemini_call_attempt_failed", {
        attempt,
        error: err instanceof Error ? err.message : String(err),
      });
    }
  }
  throw lastErr instanceof Error ? lastErr : new Error(`gemini image: exhausted 3 attempts — ${String(lastErr)}`);
}
...
export async function editDriveImageBackground(
  ...
  // Trailing clauses: keep the same viewing angle from the reference,
  // and only add motion effects (blurred wheels, streaking road) when
  // the scene itself describes motion. Prevents "parked car with
  // blurred wheels" artifacts.
  const isMotionScene = /\b(driving|driv[ea]|cornering|mid-corner|highway|track|racing|apex|panning|action|motion)\b/i.test(slotPrompt);
...
**STEP -1 — COOKWARE/UTENSIL PHYSICS (default rules).**
- **A hot cast iron / stainless / carbon-steel pan NEVER sits on bare wood.** Wood scorches under a hot pan. Any "cooking in progress" frame puts the pan on the STOVE (gas grate / induction / coil). A finished dish uses the plate. If a pan appears off-stove in a frame, it sits on a TRIVET, a stone slab, an induction mat, or a cast iron rack — NEVER directly on a butcher block, wooden cutting board, or wooden table.
- **Egg-cooking utensil = SILICONE / RUBBER SPATULA, NOT a wooden spoon.** For eggs (scrambled, fried, omelet), the correct tool is a soft silicone/rubber spatula that won't scratch a nonstick surface AND doesn't stick to the egg. Do NOT describe a wooden spoon in an egg-cooking frame. Wooden spoons belong in stew pots and pasta water — not in a scrambled egg pan.
- **Match utensils to the cookware:** wooden spoons for stainless / enameled cast iron / dutch oven simmering; silicone spatulas for nonstick and cast iron with delicate proteins (fish, eggs); metal spatulas for smash burgers on carbon-steel griddles.
- **THE COOKWARE TYPE MUST BE EXPLICIT AND CORRECT.** If the move title or body names a specific pan (wok, dutch oven, cast iron skillet, carbon-steel skillet, saute pan, saucier, sheet pan, roasting pan, stockpot, paella pan, tagine, donabe), the frame MUST show THAT SPECIFIC pan — not a generic substitute. Distinguishing features to lock in:
  - **WOK.** Round-bottom carbon-steel or seasoned iron, deep-curved sides, long single wooden handle plus a small helper handle, dark seasoned patina, sitting on a gas ring or wok ring over a burner. NOT a flat-bottom stir-fry skillet. NOT a saute pan.
  - **DUTCH OVEN.** Round or oval enameled cast iron, tight-fitting domed or flat lid, small side handles on the pot, brand-color exterior (Le Creuset flame orange, Staub matte black or graphite, Lodge black), heavy cast-iron interior (or black enamel). NOT a plain skillet with a lid.
  - **CAST IRON SKILLET.** Pre-seasoned black surface, thick pour-lip spouts on two sides, single long forged handle, uniformly thick walls. NOT stainless. NOT enameled.
  - **CARBON-STEEL SKILLET.** Similar shape to cast iron but LIGHTER, hammered or smooth silvery-blue-black patina, single long handle (often forged or riveted). NOT the thick-walled matte cast iron look.
  - **SAUTE PAN.** Straight vertical sides, wide flat bottom, long handle plus helper handle, tight-fitting flat lid, stainless clad or hard-anodized. NOT a skillet with sloped sides.
  - **ROASTING PAN.** Large rectangular or oval, low sides (2-3 inches), two riveted side handles, heavy stainless-clad or dark carbon-steel bottom, often with a v-rack. NOT a sheet pan. NOT a casserole dish.
  - **SHEET PAN.** Flat rectangular half-sheet or quarter-sheet, aluminum with a rolled rim ~1 inch tall, no handles, no legs. NOT a roasting pan.
  - **PAELLA PAN.** Very wide, very shallow (1-2 inches), two small riveted side handles, black or polished-steel bottom, uniquely broad diameter (14-18 inches typical). NOT a saute pan.
  If the frame renders a saute pan when the move said wok, or a cast iron when the move said carbon-steel, that is an automatic fail. The reader sees the mismatch immediately.

**STEP 0 — NO PROBES, THERMOMETERS, OR INSTRUMENTS IN THE FOOD (default).** By default, do NOT describe or include a probe thermometer, instant-read thermometer, meat probe, cake tester, or any measuring instrument stuck INTO the food, sitting IN the pan next to the food, or resting ON the plate. Cooks use thermometers off-camera; the finished shot doesn't need one. The ONLY exception: if the move title itself explicitly names temperature (e.g. "The 130°F Slow-Cook Salmon", "Reading Internal Temp on the Reverse Sear", "The Thermometer Method for Custard") then the probe IS the point and belongs in the frame. In every other case, describe the food WITHOUT any instrument in it.
...
export async function generateTastingImageWithReference(
  ...
  if (kind === "drink") {
    // Gemini background-only edit around the retailer bottle photo.
    // callGemini itself does 3-attempt retry on transient failure; if
    // it still throws after that, ship the raw retailer ref as a last
    // resort so the slot never renders empty.
    try {
      ...
      if (drinkRef) {
        ...
        try {
          ...
          console.info("latte.drink_shipped_gemini_edited", { subject, sourceUrl: drinkRef.sourceUrl });
    ...
    try {
      ...
      if (productRef) {
        ...
        try {
          ...
          console.info("latte.product_shipped_gemini_edited", { subject, sourceUrl: productRef.sourceUrl });
...
**LEGIBILITY FLOOR — HARD RULE.** This image ships as a ~600px-wide thumbnail inside a newsletter, viewed at coffee-shop / phone brightness. Even if the actual film is famously dark, chiaroscuro, or low-key (The Penguin, The Batman, Sicario, No Country for Old Men, Se7en, film-noir stylistics), the still MUST have enough exposure and contrast to be READABLE at thumbnail scale. That means: primary subject faces / silhouettes clearly visible against the background, key elements at least mid-tone (not crushed to pure black), a light source visible in the frame or clearly implied. If the source film's actual look is too dark for thumbnail legibility, LIFT the shadows a stop or two in your imagined shot — match the FILM'S PALETTE (warm sodium, blue steel, sickly green) but at a brightness the reader can actually parse. A frame that renders as a solid black rectangle in email is a failure regardless of stylistic accuracy.`
...
**ARCHITECTURAL COHERENCE — HARD RULE.** The poster hangs on a REAL, COMPLETE wall or a REAL, COMPLETE poster surface. NO half-walls, NO free-floating wall segments, NO walls with a nonsensical rectangular opening cut through them, NO walls that terminate in mid-air, NO M.C. Escher partitions. If the setting is an art-house lobby, the walls are complete lobby walls with a normal doorway (framed, with jambs, at floor level) or no doorway at all. If the setting is a residential home, walls are floor-to-ceiling with real trim, and the poster is centered on a flat section of wall — not on some hybrid pony-wall / room-divider / mystery partition. When in doubt, simplify: a single flat wall behind the poster, one clear light source, no invented architecture. Additional posters in the same frame (a wall of posters) is fine ONLY if the wall itself is a real complete wall — no random half-walls with sub-posters mounted on them.`
      : kind === "book"
