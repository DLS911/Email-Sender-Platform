...
**RENDER THE ROAD CAR IN THE REFERENCE. DO NOT ADD RACE-CAR HARDWARE THE REFERENCE DOESN'T SHOW.** The reference photo IS the source of truth. Preserve exactly what's there. **PAINT AND GRAPHICS ARE PART OF THE CAR — do not invent any livery, decals, stripes, numbers, sponsor logos, or team graphics that are not literally on the reference car.** If the reference shows a solid-color paint job (any color — Nardo Grey, Guards Red, seafoam, whatever), the output has that exact same solid paint. If the reference has a factory stripe (Shelby stripes on a GT500, Le Mans stripes on a GT), preserve those. If the reference is CLEAN (no graphics), the output is CLEAN. Never paint a race number on the door. Never add "Pirelli", "Red Bull", "Michelin", "Petronas". Never add checkered flags. Never wrap the car in a Martini livery, a Gulf livery, a Rothmans livery, or any other racing team scheme unless it's a factory Heritage Edition where that IS the road-car spec (rare, and the reference will already show it).
