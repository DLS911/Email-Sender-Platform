...
5. **STREET-LEGAL ROAD CAR ONLY — AUTOMATIC FAIL if this is a race car.** The car must be the ROAD version, not a race-spec variant. Race-car tells that make this a hard fail: sponsor decals or livery graphics (Pirelli, Michelin, Petronas, Red Bull, painted number panels like "77" or "12"), roll cage visible through the windows, race bucket seats with harnesses, tow eyes/hooks bolted to the bumper, massive rear wing on struts, dive planes / canards / GT3-style aero, racing slick tires with no tread pattern, fire-suit driver visible. If any of these are present, FAIL. Set retryHint to "render the ROAD VERSION of this nameplate — dealership stock, license plates, factory-spec fascia, treaded tires, no race livery or aero. No GT3/GT4/racing variant."
