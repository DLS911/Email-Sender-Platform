...
2b. **BORING-OBJECT AUTOMATIC FAIL.** If the frame is a plain, uninteresting object on a plain surface with no rich texture, no evocative light, and nothing happening — FAIL. Examples that FAIL on this rule: a bare metal window latch on white paint, a plain wooden door with no light or context, an empty chair against a blank wall, a doorknob, a mailbox by itself, a shop sign with no scene around it, a plain condensation-only window, a plain empty street corner. The frame must EARN attention with rich texture (weathered brass, wet cobblestone in raking light, worn leather, layered candle wax, molten glass, layered spice jars, jamón being sliced) OR unusual light/color (gold-hour through amber whisky, cobalt tile catching sun, Northern lights on snow, saffron on marble) OR something quietly happening (a butcher trimming, a baker turning dough, an old radiator hissing steam next to a book, wax being sealed on a bottle). Set retryHint to "add rich texture, evocative light, or a quiet human action to make the frame captivating — no more plain-object-on-plain-surface renders."
