...
4. **Steam physics is an AUTOMATIC FAIL if wrong.** Steam ONLY escapes from open apertures. This applies to EVERY covered vessel in the frame:
   - Kettle with a closed lid → NO steam from the lid top, lid seam, or vessel sides. Pour-over gooseneck kettles (Fellow Stagg, Hario Buono, Chemex-adjacent) always have closed lids.
   - **Dutch oven / cocotte / cast-iron pot / stockpot with the lid ON → NO steam from the lid, no wisps around the lid perimeter, no plume rising from the closed top.** If steam is visible, the lid must be visibly OFF (leaning against the pot / on a trivet nearby) or visibly AJAR (tilted with a gap). Le Creuset / Staub / Lodge dutch ovens have snug-fitting lids — closed lid = sealed vessel = no steam.
   - Saute pan / skillet with a lid on → same rule.
   - Steam CAN come from: an OPEN spout, a REMOVED lid, an uncovered pot, an open cup of hot liquid, actively simmering water in an uncovered vessel, hot food on a plate, freshly poured hot liquid landing in another vessel.
   Set retryHint to "vessel lid is CLOSED but steam is venting from it — either remove all steam plumes (lid stays closed) or show the lid visibly off / ajar to justify the steam".
...
- If wrong: {"ok": false, "reason": "specific failure", "retryHint": "what to fix"}`;
    ...
    case "coverDetail":
      return `This image is a Cover Story Detail — a SECOND SCENIC SHOT of ${ctx.subject}, complementing the Hero from a different angle or with a different subject. It's a companion location image, NOT a tight object still-life.

Check the following:
1. Does the frame feel specific to ${ctx.subject} (not a generic version of the region)?
2. **TIGHT-OBJECT AUTOMATIC FAIL.** If the image is a close-up of a doorknob, a window latch, a mailbox, a chair against a wall, a bare shop sign with no scene around it, an isolated object on a plain surface, or any "detail of a single object" render — FAIL. Cover Detail is a SCENIC location shot, not an editorial object still-life. Set retryHint to "widen the shot into a real scene of ${ctx.subject} — a vista / a landmark / the story's signature subject (wildlife / festival / activity) in frame, not a cropped object detail."
3. **GENERIC-SCENE AUTOMATIC FAIL.** Street food stand, food stall, night market crowd, busy morning market, bustling plaza, generic downtown crowd, people milling around — FAIL. Set retryHint to "pick a specific named scene of ${ctx.subject} — a signature landmark, a piece of coastline, the wildlife the story is about, the festival subject in traje — not a generic crowd scene."
